...
describe('Sms API:', function() {
  ...
  describe('GET /hook/sms', function() {
    var smss;
  ...
  describe('POST /hook/sms', function() {
    beforeEach(function(done) {
      request(app)
        .post('/hook/sms')
        .send({
  ...
  describe('GET /hook/sms/:id', function() {
    var sms;

    beforeEach(function(done) {
      request(app)
        .get('/hook/sms/' + newSms._id)
        .expect(200)
        .expect('Content-Type', /json/)
        .end((err, res) => {
          if (err) {
  ...
  describe('PUT /hook/sms/:id', function() {
    var updatedSms;

    beforeEach(function(done) {
      request(app)
        .put('/hook/sms/' + newSms._id)
        .send({
          name: 'Updated Sms',
  ...
  describe('DELETE /hook/sms/:id', function() {

    it('should respond with 204 on successful removal', function(done) {
      request(app)
        .delete('/hook/sms/' + newSms._id)
        .expect(204)
        .end((err, res) => {
          if (err) {
            return done(err);
